...
+ (BOOL)saveAuthorization:(TwitchAppFetcherAuthorization *)auth toKeychainForName:(NSString *)keychainItemName {
    NSData *data = [NSKeyedArchiver archivedDataWithRootObject:auth];
    return [GTMKeychain savePasswordDataToKeychainForName:keychainItemName passwordData:data];
}

+ (BOOL)removeAuthorizationForKeychainForName:(NSString *)keychainItemName {
    return [GTMKeychain removePasswordFromKeychainForName:keychainItemName];
}

+ (instancetype)authorizationFromKeychainForName:(NSString *)keychainItemName {
    NSData *data = [GTMKeychain passwordDataFromKeychainForName:keychainItemName];
    if (!data) {
        return nil;
    }
    
    TwitchAppFetcherAuthorization *auth = (TwitchAppFetcherAuthorization *)[NSKeyedUnarchiver unarchiveObjectWithData:data];
